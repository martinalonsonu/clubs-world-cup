...
const MatchesByGroup = props => {
    ...
    const { group, errors, handleSubmitForm, matches, onSubmitForm, register, teams, urlAssets } = props;
    ...
    const getBgColor = (matchDate, today, finished) => {
        ...
        const nextMatchDate = dayjs(`${dayjs().year()}-${getFormattedNumberMonth(splittedMatchDate[3])}-${splittedMatchDate[1]}`);
    ...
    return (
        <section className="">
            <h2 className="my-2 text-center group-name">GRUPO {group}</h2>
                        ...
                        return (
                            <Fragment key={match.matchNumber}>                                   
                                ...
                                <GroupStageTeam 
                                    ...
                                    bgColor={getBgColor(match.matchDate, today, match.finished)}
                                ...
                                <article className="col-12 d-flex align-items-center justify-content-center mb-2 text-center">
                                    <span className="match-data">{match.stadium} - {`${(match.matchDate).charAt(0).toUpperCase()}${(match.matchDate).slice(1)}`} - {match.matchHour}</span>
                                </article>
                            </Fragment>                    
                        )})
                    }
