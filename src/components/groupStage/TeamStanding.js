...
const TeamStanding = props => {
    ...
    const { positionClass, slug, name, playedMatches, wonMatches, drawnMatches, lostMatches, goalsFor, goalsAgainst, goalsDifference, points, urlAssets } = props;

    const CellValue = ({children}) => <td><span>{children}</span></td>;

    return (
        <tr className={`mb-1 standing-data ${positionClass}`}>
            <td>
                <article className="d-flex align-items-center justify-content-start my-1 ms-1">
                    <img className="px-1" src={`${urlAssets}/img/escudos/${slug}.png`} alt={name} loading="lazy" />
