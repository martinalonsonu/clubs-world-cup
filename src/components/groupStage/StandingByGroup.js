...
const StandingByGroup = props => {
    ...
    const ColumnName = ({children}) => <th scope="col"><span className="mx-1">{children}</span></th>;

    const getPositionColor = pos => {

        const positionColors = ['text-selected to-knockout-stage-team', 'text-selected to-knockout-stage-team', 'text-unselected to-europe-league-team', 'text-unselected eliminated-team']
        ...
        <section className="mt-2 mb-4 mb-md-5 px-1">
            <table className="w-100 table text-center">
                <thead className="standing-head">
                    <tr>
                        <th scope=""><span className="mx-5"></span></th>
